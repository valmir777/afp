...
<p><?php _e( 'Alguns dos itens que você encomendou foram enviados. Os itens que foram enviados são os seguintes:',  'dc-woocommerce-multi-vendor' ); ?></p>

<?php do_action( 'woocommerce_email_before_order_table', $order, true ); ?>

<h2><?php printf( __( 'Pedido: %s',  'dc-woocommerce-multi-vendor' ), $order->get_order_number() ); ?> (<?php printf( '<time datetime="%s">%s</time>', date_i18n( 'c', strtotime( $order->get_date_created() ) ), date_i18n( wc_date_format(), strtotime( $order->get_date_created() ) ) ); ?>)</h2>
...
<table cellspacing="0" cellpadding="6" style="width: 100%; border: 1px solid #eee;" border="1" bordercolor="#eee">
	<thead>
		<tr>
			...
			<th scope="col" style="text-align:left; border: 1px solid #eee;"><?php _e( 'Preço',  'dc-woocommerce-multi-vendor' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php 
...
<h2><?php _e( 'Detalhes do rastreamento de remessa',  'dc-woocommerce-multi-vendor' ); ?></h2>
<p><strong><?php _e( 'Url de rastreio:',  'dc-woocommerce-multi-vendor' ); ?></strong> <?php echo $tracking_url; ?></p>
<p><strong><?php _e( 'Id de rastreio:',  'dc-woocommerce-multi-vendor' ); ?></strong> <?php echo $tracking_id; ?></p>

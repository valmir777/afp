...
<?php 
	if($transaction_mode == 'paypal_masspay') {
		echo apply_filters( 'wcmp_admin_direct_bank_received_text', sprintf(__( 'Olá,%s completou com sucesso uma retirada de $%s em %s através do PayPal. Os detalhes do pedido são os seguintes:', 'dc-woocommerce-multi-vendor'), '<a href='.$vendor->permalink.'>'.$vendor->page_title.'</a>', $amount, get_the_date( 'd/m/Y', $transaction_id )), $transaction_id ); 
	} else if($transaction_mode == 'direct_bank'){
		echo apply_filters( 'wcmp_admin_paypal_received_text', sprintf(__( 'Olá,<br> Existe um novo pedido de levantamento de $%s de um fornecedor %s no seu site. Os detalhes do pedido são os seguintes:', 'dc-woocommerce-multi-vendor'), $amount, '<a href='.$vendor->permalink.'>'.$vendor->page_title.'</a>'), $transaction_id );
	}
?>
</p>
